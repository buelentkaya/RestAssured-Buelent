...
String requestJsonBody="{\"gender\":\"Male\",\n" +
        "\"name\":\"Severus\",\n" +
        "\"phone\":8877445596}";
        Response post = given().accept(ContentType.JSON)//what we are asking from api which isJSON response
                .and().contentType(ContentType.JSON)//what we are sending to api
        ...
        assertThat(post.path("success"),is(expectedResponseMessage));
        assertThat(post.path("data.name"),is("Severus"));
        assertThat(post.path("data.gender"),is("Male"));
      assertThat(post.path("data.phone"),is(8877445596L));
